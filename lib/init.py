...
def has_goldkash_conf():
    import config
    import io

    valid_goldkash_conf = False

    # ensure goldkash_conf exists & readable
    #
    # if not, print a message stating that GoldKash Core must be installed and
    # configured, including JSONRPC access in goldkash.conf
    try:
        f = io.open(config.goldkash_conf)
        valid_goldkash_conf = True
    except IOError as e:
        print(e)

    return valid_goldkash_conf
...
def main():
    ...
    if not has_goldkash_conf():
        print("GoldKashCore must be installed and configured, including JSONRPC access in goldkash.conf")
        sys.exit(1)
