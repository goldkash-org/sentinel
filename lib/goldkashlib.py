...
def is_valid_goldkash_address(address, network='mainnet'):
    ...
    goldkash_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid GoldKash address
        return False

    if (address_version != goldkash_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.goldkashd_serialise())
...
def SHIM_serialise_for_goldkashd(sentinel_hex):
    ...
    # superblock => "trigger" in goldkashd
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # goldkashd expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    goldkashd_hex = serialise(obj)
    return goldkashd_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('goldkash.conf')
