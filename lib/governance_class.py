...
class GovernanceClass(object):
    ...
    def vote(self, goldkashd, signal, outcome):
        return self.go.vote(goldkashd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, goldkashd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(goldkashd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(goldkashd, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import goldkashlib
        obj_data = goldkashlib.SHIM_serialise_for_goldkashd(self.serialise())
    ...
    def goldkashd_serialise(self):
        import goldkashlib
        return goldkashlib.SHIM_serialise_for_goldkashd(self.serialise())
