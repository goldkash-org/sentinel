...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, goldkashd):
        golist = goldkashd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_goldkashd(goldkashd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_goldkashd(self, goldkashd, rec):
        import decimal
        import goldkashlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from goldkashd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from goldkashd, with every run
        ...
        # get/create, then sync payment amounts, etc. from goldkashd - GoldKashd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(goldkashd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from goldkashd! %s" % e)
            govobj.vote_delete(goldkashd)
            return (govobj, None)
    ...
    def vote_delete(self, goldkashd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(goldkashd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, goldkashd, signal, outcome):
        import goldkashlib
        ...
        output = goldkashd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = goldkashlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(goldkashd, signal)

    def sync_network_vote(self, goldkashd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = goldkashd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = GOLDKASHD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 goldkash addr, non-multisig
            if not goldkashlib.is_valid_goldkash_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid GoldKash address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # GoldKash Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (goldkashlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = GOLDKASHD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not goldkashlib.is_valid_goldkash_address(addr, config.network):
    ...
    def hash(self):
        import goldkashlib
        return goldkashlib.hashit(self.serialise())
