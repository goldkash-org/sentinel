...
sentinel_cfg = GoldKashConfig.tokenize(sentinel_config_file)
sentinel_version = "1.2.0"
min_goldkashd_proto_version_with_sentinel_ping = 70207


def get_goldkash_conf():
    if sys.platform == 'win32':
        goldkash_conf = os.path.join(os.getenv('APPDATA'), "GoldKashCore/goldkash.conf")
    else:
        home = os.environ.get('HOME')

        goldkash_conf = os.path.join(home, ".goldkashcore/goldkash.conf")
        if sys.platform == 'darwin':
            goldkash_conf = os.path.join(home, "Library/Application Support/GoldKashCore/goldkash.conf")

    goldkash_conf = sentinel_cfg.get('goldkash_conf', goldkash_conf)

    return goldkash_conf
...
goldkash_conf = get_goldkash_conf()
network = get_network()
