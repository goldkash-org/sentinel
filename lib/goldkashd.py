...
class GoldKashDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some goldkashd methods
    ...
    @classmethod
    def from_goldkash_conf(self, goldkash_dot_conf):
        from goldkash_config import GoldKashConfig
        config_text = GoldKashConfig.slurp_config_file(goldkash_dot_conf)
        creds = GoldKashConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = goldkashlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = goldkashlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = goldkashlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_goldkashd_proto_version_with_sentinel_ping)
